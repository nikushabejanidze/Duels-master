...
public class DuelManager implements Loadable {
    ...
    // NEW: combat/region hooks from the "new" code
        ...
        if (config.getMaxDuration() > 0) {
            this.durationCheckTask = plugin.doSyncRepeat(() -> {
                for (final ArenaImpl arena : arenaManager.getArenasImpl()) {
                    ...
                    // Only handle undecided matches (not endgame) that exceeded max duration
    ...
    /**
     * Extra pre-start checks from the new code path, adapted to our existing API.
     */
    private boolean extraPrechecks(final Collection<Player> players, final Settings settings) {
        // 1) dead players
        ...
        // 2) blacklisted worlds
        for (Player p : players) {
            if (isBlacklistedWorld(p)) {
        ...
        // 4) cancel if moved (base location check)
        if (config.isCancelIfMoved()) {
            for (Player p : players) {
                final Location base = settings.getBaseLoc(p);
                if (notInLoc(p, base)) {
        ...
        // 5) duelzone (WorldGuard) check
        if (config.isDuelzoneEnabled() && worldGuard != null) {
            for (Player p : players) {
                final String dz = settings.getDuelzone(p);
                if (notInDz(p, dz)) {
    ...
    private boolean notInLoc(Player player, Location location) {
        ...
        return !Objects.equals(source.getWorld(), location.getWorld())
    ...
    private boolean notInDz(Player player, String duelzone) {
        return duelzone != null && worldGuard != null && !duelzone.equals(worldGuard.findDuelZone(player));
    }
    ...
    /**
     * Resets a player's state on tie.
     */
    ...
    public boolean startMatch(final Collection<Player> first, final Collection<Player> second, final Settings settings, final Map<UUID, List<ItemStack>> items, final Queue source) {
        ...
        // NEW: extra checks drawn from the new code
        if (!extraPrechecks(players, settings)) {
            refundItems(players, items);
            return false;
        }
    ...
    private void addPlayers(final Collection<Player> players, final DuelMatch match, final ArenaImpl arena, final KitImpl kit, final Location location) {
        for (final Player player : players) {
            ...
            if (kit != null) {
                PlayerUtil.reset(player);
                // player-specific kit overwrite
                com.meteordevelopments.duels.kit.edit.KitEditManager kem = com.meteordevelopments.duels.kit.edit.KitEditManager.getInstance();
                if (kem != null && kem.hasPlayerKit(player, kit.getName())) {
                    kem.loadPlayerKit(player, kit.getName());
                } else {
                    kit.equip(player);
                }
            }
    ...
    // Optional: clickable inventory viewers like in the new code
    private void sendInventoryViewLinks(final DuelMatch match) {
        if (!config.isDisplayInventories()) return;
        final String color = lang.getMessage("DUEL.inventories.name-color");
        final TextBuilder builder = TextBuilder.of(lang.getMessage("DUEL.inventories.message"));
        final Iterator<Player> it = match.getAllPlayers().iterator();
        while (it.hasNext()) {
            Player p = it.next();
            builder.add(StringUtil.color(color + p.getName()), ClickEvent.Action.RUN_COMMAND, "/duel _ " + p.getUniqueId());
            if (it.hasNext()) builder.add(StringUtil.color(color + ", "));
        }
    ...
    public void handleMatchEnd(DuelMatch match, ArenaImpl arena, Player loser, Location deadLocation, Player winner) {
        DuelsPlugin.getMorePaperLib().scheduling().regionSpecificScheduler(arena.first().getLocation()).runDelayed(() -> {
            if (arena.size() == 0) {
                ...
                plugin.doSyncAfter(() -> {
                    inventoryManager.handleMatchEnd(match);
                    sendInventoryViewLinks(match);
                }, 1L);
                ...
                if (config.isEndCommandsEnabled() && !(!match.isFromQueue() && config.isEndCommandsQueueOnly())) {
                    try {
                        for (final String command : config.getEndCommands()) {
                            int delay = 0;
                            String cmd = command;
                            // Support {delay:xxx} token like the new code
                            int idxStart;
                            int idxEnd;
                            if (command.contains("{delay:") && (idxEnd = command.indexOf("}", idxStart = command.indexOf("{delay:") + 7)) > idxStart) {
                                String delayStr = command.substring(idxStart, idxEnd);
                                try { delay = Integer.parseInt(delayStr); } catch (NumberFormatException ignored) { Log.warn(this, "Invalid delay format in command: " + command); }
                                cmd = command.substring(0, idxStart - 7) + command.substring(idxEnd + 1);
                            }
        ...
        DuelsPlugin.getMorePaperLib().scheduling().regionSpecificScheduler(arena.first().getLocation()).runDelayed(() -> {
            ...
            DuelsPlugin.getMorePaperLib().scheduling().entitySpecificScheduler(arena.first()).runDelayed(() -> {
                ...
                if (config.isEndCommandsEnabled() && !(!match.isFromQueue() && config.isEndCommandsQueueOnly())) {
                    try {
                        for (final String command : config.getEndCommands()) {
                            int delay = 0; String cmd = command;
    ...
    private class DuelListener implements Listener {
        ...
        public void on(final PlayerDeathEvent event) {
            ...
            if (mcMMO != null) mcMMO.enableSkills(player);
            final DuelMatch match = arena.getMatch();
            if (match == null) return;

            // Skip death handling for ROUNDS3 as it's handled elsewhere
            ...
            if (prevSize < 2 || match.size() >= prevSize) return;
            final Location deadLocation = player.getEyeLocation().clone();
        ...
        public void on(final EntityDamageEvent event) {
            ...
            final ArenaImpl arena = arenaManager.get(player);
            if (arena == null || !arena.isEndGame()) return;
            event.setCancelled(true);
        }

        @EventHandler
        public void on (final PlayerInteractEvent event) {
            if (!config.isDisableEnderpearlInEndgame()) return;
        ...
        public void on(final PlayerMoveEvent event) {
            ...
            final ArenaImpl arena = arenaManager.get(player);
            if (arena == null) return;
            if (!config.isDisableMovementInEndgame()) return;
            if (!arena.isEndGame()) return;
            event.setCancelled(true);
        }
        ...
        public void on(final PlayerDropItemEvent event) {
            ...
            if (!arenaManager.isInMatch(event.getPlayer())) return;
        ...
        public void on(final PlayerPickupItemEvent event) {
            // Let Loyalty tridents return even if pickup is blocked
            if (!CompatUtil.isPre1_13() && event.getItem().getItemStack().getType() == Material.TRIDENT) return;
            if (!config.isPreventItemPickup() || !arenaManager.isInMatch(event.getPlayer())) return;
            event.setCancelled(true);
        }
        ...
        public void on(final InventoryOpenEvent event) {
            if (!config.isPreventInventoryOpen()) return;
