...
/**
 * Merged QueueSignManagerImpl
 * - Keeps old features: Permissions constant, sign text with queue name, Lang-aware constructor,
 *   NIO JSON, same events & scheduling.
 * - Compatible with new data loader and periodic sign updates.
 */
public class QueueSignManagerImpl implements Loadable, QueueSignManager, Listener {
    ...
    public boolean create(final Player creator, final Location location, final Queue queue) {
        ...
        // Keep old: include queue name in sign text and pass Lang to constructor
        signs.put(location, created = new QueueSignImpl(
                location,
                lang.getMessage("SIGN.format", "name", queue.getName(), "kit", kitName, "bet_amount", queue.getBet()),
                queue,
                lang
        ));
